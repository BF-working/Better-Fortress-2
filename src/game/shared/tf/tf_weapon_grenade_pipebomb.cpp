...
void CTFGrenadePipebombProjectile::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( m_iType == TF_GL_MODE_REGULAR || m_iType == TF_GL_MODE_CANNONBALL )
	{
		...
		// Blow up if we hit an enemy we can damage
		else if ( pHitEntity->GetTeamNumber() && ( pHitEntity->GetTeamNumber() != GetTeamNumber() || friendlyfire.GetBool() ) && pHitEntity->m_takedamage != DAMAGE_NO /*|| pHitEntity->m_nTFFlags & PROJECTILES_EXPLODE_ON_TOUCH*/)
		{
	...
	if ( pHitEntity && ( pHitEntity->IsWorld() || bIsDynamicProp || (pHitEntity->m_nTFFlags & STICKS_PROJECTILES) ) && gpGlobals->curtime > m_flMinSleepTime )
	{
		...
		if ( pHitEntity->m_nTFFlags & STICKS_PROJECTILES )
